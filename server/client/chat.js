...
async function appendFile(sender, fileId, filename, mimeType, time) {
  ...
  document.getElementById('btn-tasks').addEventListener('click', () => {
    const frame = document.getElementById('modal-tasks-frame');
    frame.src = '/miniapps/tasks/index.html';
    document.getElementById('modal-tasks').style.display = 'flex';
  });

  // Закрытие
  document.getElementById('modal-tasks-close').addEventListener('click', () => {
    document.getElementById('modal-tasks').style.display = 'none';
    document.getElementById('modal-tasks-frame').src = '';
  });

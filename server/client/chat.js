...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const roomMeta = {}; 
  const NEWS_API_KEY = 'bcc4b3b9b8b144c7af731ceb0a8c7001';
...
async function appendFile(sender, fileId, filename, mimeType, time) {
  ...
  document.getElementById('btn-weather').addEventListener('click', () => {
    ...
    frame.src = '/miniapps/weather/index.html';
...
// Открыть модалку и загрузить новости
newsBtn.onclick = async () => {
  newsListEl.innerHTML = '<p>Загрузка...</p>';
  newsModal.classList.remove('hidden');
  try {
    const res = await fetch(
      `https://newsapi.org/v2/top-headlines?country=ru&pageSize=10&apiKey=${NEWS_API_KEY}`
    );
    const { articles } = await res.json();
    if (!articles || !articles.length) {
      newsListEl.innerHTML = '<p>Нет доступных новостей.</p>';
      return;
    }
    ...
    console.error(err);
    newsListEl.innerHTML = '<p>Ошибка при загрузке новостей.</p>';
  }
};
...
newsModal.querySelector('.modal-overlay').onclick = () => {
  newsModal.classList.add('hidden');
};